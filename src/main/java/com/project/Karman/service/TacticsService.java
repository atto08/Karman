...
public class TacticsService {
    ...
    // 카테고리 - formation
    @Transactional
    public AiCoachResponseDto askTacticalCoach(AskTacticsRequestDto requestDto) {

        log.info("---formation: {} & subtopic: {}", requestDto.formation(), requestDto.subtopic());
        // 유사도 검색 - metadata로 범위 축소
        List<Document> documents = documentService.similaritySearch(requestDto);
        ...
        Prompt prompt = openAiService.createPrompt(
                ...
                PromptMessage.ASK_TACTICS_FORMATION.getMessage(),
                context.toString());
        // Ai 응답 생성 - 사용자 요청
