...
public class ClubService {
    ...
    @Transactional(readOnly = true)
    public PlayerSelectListResponseDto findPlayerSelectList(Member member, UUID clubId) {
        // 클럽 존재 여부 확인
        if (!clubRepository.existsById(clubId)) {
            throw new CustomException(ExceptionMessage.NOT_FOUND_CLUB);
        }
    ...
    @Transactional(readOnly = true)
    public ClubStatisticsRecordsResponseDto getClubStatistics(Member member, UUID clubId) {
        // 클럽 조회
        if (!clubRepository.existsById(clubId)) {
            throw new CustomException(ExceptionMessage.NOT_FOUND_CLUB);
        }
